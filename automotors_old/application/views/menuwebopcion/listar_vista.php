<?php echo form_open('menuwebopcion',array( 'id' => 'ff','data-options'=>'ajax:false')); ?> 
<div class="easyui-panel" style="width:100%;max-width:1000px;padding:30px 60px;">
    ...
    <table id="tt" class="easyui-datagrid" width="100%"
        ...
        <thead>
            <tr>
                <th field="menu_web_opcion" width="80">ID menu web opcion</th>
    ...
    <div id="toolbar">
    	<?php if(isset($acc_registrar) && $acc_registrar == 'MWO_I'):?>
...
<script>
    ...
    function BotonesAccion(value,row) {
        ...
        //var accionVer = 'window.location.href = \'<?= base_url() ?>menuwebopcion/detalle_form/'+row.menu_web_opcion+'\'';
        var accionEditar = 'window.location.href = \'<?= base_url() ?>menuwebopcion/actualizar_form/'+row.menu+ '/'+ row.menu_web_opcion+'\'';
        var accionEliminar='eliminar(\''+row.menu_web_opcion+'\',\''+row.nombre_opcion+'\')';
...
//		return btnEditar + btnEliminar + btnActivar;
    ...
    function eliminar(id,nombre) {
        ...
        if (r == true) {
            window.location.href = "<?= base_url() ?>menuwebopcion/eliminar/"+id;
        } 
    }
    ...
    function activar(id,nombre) {
        ...
        if (r == true) {
            window.location.href = "<?= base_url() ?>menuwebopcion/activar/"+id;
        } 
    }
    
    function nuevo_registro() {
       window.location.href = "<?= base_url() ?>menuwebopcion/insertar_form/"+ $('#cc').val(); 
    }
</script>