...
class MenuWeb extends CI_Controller
{
    ...
    public function Index($mensaje=null) {		
		$data['json'] = json_encode($this->menuweb_model->Lista());
		$data['mensaje']=$mensaje;
		
		//Registrar
		$opcion = 'MW_I';
		if(verificar_acceso($this->menuId, $opcion)) {
        	$data['acc_registrar'] = "MW_I";
		} else {
		...
		//Asignando vista
		$this->vista->SetView("menuweb/listar_vista",$data);
    }
    
    private function _generarAbm($salida = null) {
		$this->vista->SetView("menuweb/listar_vista",(array)$salida);
	}
	
	public function agregar_menu_web($mensaje=null) {
		$data["operacion"]="i";
	...
	public function editar_menu_web($id,$mensaje=null) {
		$data['menu'] = $this->menuweb_model->Get_menu($id);
		$data["operacion"]="u";
	...
	private function _operacion_menu($data) {
		$data["json"]=json_encode($this->menuweb_model->Lista_padre());
		$this->load->helper('form');
		$this->vista->SetView("menuweb/formulario_vista",$data);
	}
	
	public function guardar_menu_web($id=null) {
		$datos=array();
		$datos=array(
		...
		$resultado = $this->menuweb_model->Abm($datos);
		
		if($this->input->post('operacion')=="i")
		{
			$this->agregar_menu_web($resultado);
		}
		else if($this->input->post('operacion')=="u"){
			$this->editar_menu_web($this->input->post('MENU'),$resultado);
		}
		
	}
	
	public function eliminar_menu_web($id=null) {
		if(isset($id)){
			$datos=array(
				"d",
				$id,
	...
	public function activar_menu_web($id=null) {
		if(isset($id)){
			$datos=array(
				"a",
				$id,
