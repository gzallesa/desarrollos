<?php echo form_open('acceso/guardar',array( 'id' => 'ff','data-options'=>'ajax:false')); ?> 
    <div class="easyui-panel" title="<?= $titulo ?>" style="width:100%;max-width:600px;padding:30px 60px;"> 
        <div style="margin-bottom:20px">
            ...
            <input class="easyui-textbox" label="Cod Acceso:" labelPosition="top" style="width:100%;height:52px"  name="cod_acceso" data-options="readonly:<?= isset($datos)?'true':'false'; ?>" value="<?php 
                if(isset($datos)){ echo  $datos["COD_ACCESO"];}
             ?>">
             <?php } ?>            

	        <div style="margin-bottom:20px" id="cajaCombo">
	            <input id="cb" class="easyui-combobox" name="acceso" labelPosition="top"
	    		data-options="valueField:'codigo',textField:'descripcion',data:datos,prompt:'Elija el acceso',required:true" style="width:100%;height:52px"  label="Acceso:"value="<?php 
                if(isset($datos)){ echo $datos["ACCESO"];}
             	?>">
	        </div>
    ...
    <input type="hidden" value=<?=  $cod_acceso ?> name="cod_acceso" >
