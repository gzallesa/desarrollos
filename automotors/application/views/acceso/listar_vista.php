<?php echo form_open('acceso',array( 'id' => 'ff','data-options'=>'ajax:false')); ?> 
<div class="easyui-panel" style="width:100%;max-width:1000px;padding:30px 60px;">
    ...
    <table id="tt" class="easyui-datagrid" width="100%"
            title="Acceso" iconCls="icon-save"
        ...
        <thead>
            <tr>
            	<th field="COD_ACCESO" width="80"><b>COD ACCESO</b></th>
                <th field="DESC_ACCESO" width="100"><b>ACCESO</b></th>
    ...
    <div id="toolbar">
    	<?php if(isset($acc_registrar) && $acc_registrar == 'AC_I'):?>
...
<script>
    ...
    function BotonesAccion(value,row) {
        ...
        var accionEditar = 'window.location.href = \'<?= base_url() ?>acceso/actualizar_form/'+row.COD_ACCESO+'\'';
        var accionEliminar='eliminar(\''+row.COD_ACCESO+'\',\''+row.VALOR+'\')';
...
//		return btnEditar + btnEliminar + btnActivar;
    ...
    function eliminar(id,nombre) {
        ...
        if (r == true) {
            window.location.href = "<?= base_url() ?>acceso/eliminar/"+id;
        } 
    }
    ...
    /*function activar(id,nombre) {
        ...
        if (r == true) {
            window.location.href = "<?= base_url() ?>acceso/activar/"+id;
        } 
    }*/
    ...
    function nuevo_registro() {
      ...
      if ($('#cc').val() == '') {
       ...
       window.location.href = "<?= base_url() ?>acceso/insertar_form/" + codPersonal; 
    }
</script>