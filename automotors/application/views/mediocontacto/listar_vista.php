<?php echo form_open('medioscontacto',array( 'id' => 'ff','data-options'=>'ajax:false')); ?> 
<div class="easyui-panel" style="width:100%;max-width:1000px;padding:30px 60px;">
    ...
    <table id="tt" class="easyui-datagrid" width="100%"
            title="Medios Contacto" iconCls="icon-save"
        ...
        <thead>
            <tr>
            	<th field="id_medio_contacto" width="80"><b>ID</b></th>
                <th field="medio_contacto" width="100"><b>MEDIO CONTACTO</b></th>
    ...
    <div id="toolbar">
    	<?php if(isset($acc_registrar) && $acc_registrar == 'MC_I'):?>
...
<script>
    ...
    function BotonesAccion(value,row) {
        ...
        var accionEditar = 'window.location.href = \'<?= base_url() ?>medioscontacto/actualizar_form/'+row.id_medio_contacto+'\'';
        var accionEliminar='eliminar(\''+row.id_medio_contacto+'\',\''+row.valor+'\')';
        var accionActivar='activar(\''+row.id_medio_contacto+'\',\''+row.valor+'\')';
...
//		return btnEditar + btnEliminar + btnActivar;
    ...
    function eliminar(id,nombre) {
        ...
        if (r == true) {
            window.location.href = "<?= base_url() ?>medioscontacto/eliminar/"+id;
        } 
    }
    ...
    function activar(id,nombre) {
        ...
        if (r == true) {
            window.location.href = "<?= base_url() ?>medioscontacto/activar/"+id;
        } 
    }
    ...
    function nuevo_registro() {
      ...
      if ($('#cc').val() == '') {
       ...
       window.location.href = "<?= base_url() ?>medioscontacto/insertar_form/" + codPersonal; 
    }
</script>