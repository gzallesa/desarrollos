<?php echo form_open('menumobopcion/rmm_opcion',array( 'id' => 'ff','data-options'=>'ajax:false')); ?> 
<div class="easyui-panel" style="width:100%;max-width:700px;padding:30px 60px;">
        <div style="margin-bottom:20px;text-align:center;">
			<select id="cc" class="easyui-combogrid" name="idRolMenuMob" style="width:80%" data-options="
                    panelWidth: 500,
                    idField: 'llave',
                    textField: 'rol_menu_mobile',
                    value: '<?= $valueSelected ?>',
                    url: '<?= base_url() ?>menuMobOpcion/cargarCombo',
                    columns: [[
                        {field:'llave',title:'Llave', width:40, visible:false},
                        {field:'nombre_rol',title:'Rol',width:100},
                        {field:'nombre_menu',title:'Menu',width:100}
                    ]],
                    fitColumns: true,
                    label: 'Rol Menu Mobile:',
                    labelWidth: '120px'
                ">
            </select>
        </div>
