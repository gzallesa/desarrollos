<?php
class PlanillaTotal extends CI_Controller {
	
	private $menuId = '10066';
	...
	function __construct() {
		...
		$this->load->model("planillatotal_model");
    ...
    public function Index() {
        ...
        $data['tipo_planilla'] = $this->planillatotal_model->getComboDominio($tipo_planilla,'TIPO PLANILLA');
        $data['mes'] = $this->planillatotal_model->getComboDominio($mes,'MESES');
        $data['anio'] = $this->planillatotal_model->getComboDominio($anio,'ANIO');
        $data['periodo'] = $periodo;
        $operacion = $this->input->post('operacion');
        if ($operacion == 'G') {
	        if (($tipo_planilla !== null) && ($mes !== null)&& ($anio !== null)) {
	        	$usuario = $this->session->usuario;
	        	$result = $this->planillatotal_model->getPlanilla($tipo_planilla, $periodo, $usuario);
        ...
        $this->vista->SetView("planillatotal/listar_vista", $data);
    }
