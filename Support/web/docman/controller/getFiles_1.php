...
function getFolders1(folder $folder)
{
    ...
    for($i=0;$i<$f->length();$i++)
    {
        ...
        if($_SESSION['user']->getType()=='0'or $f->getElement($i)->getUser()==$_SESSION['user']->getIDU())
        {
            echo '<tr><td><div style="float:left"><div class="foldericon"></div><a href="javascript:openfolder('.$f->getElement($i)->getIDF().');">'.$f->getElement($i)->getName().'</a><br><div style="width:500px;"><div class="cargadopor"></div>'.$u->getName().'</div></div>';
